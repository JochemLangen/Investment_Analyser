...
class Backtrace(Base):
    ...
    def backtrace_data(
        ...
        if model_type == "Osc":  # Oscillating model
            ...
            param_bounds = (
                np.array([-np.inf, -np.inf, 0, 0, min_freq, -np.inf]),
            ...
            raise ValueError(
                "The provided model type is unsuported.\n"
                + 'Either choose "Osc": '
        ...
        if ier < 1 or ier > 4:
            raise ValueError(
                "The fitting was not successful. The fit was exited with message: " + mesg
            )
        ...
        results["Residuals"] = y_resid
        results["Condition no. mag"] = condition_no_mag

        # Calculate R_adj^2:
        results["R_adj^2"] = 1 - (np.sum(y_resid**2) / (y_len - len(popt))) / (
        ...
        if model_type == "Osc":
            ...
            results["Exp-trig corr"] = np.corrcoef(np.array([exp_arr, trig_arr]))[0, 1]
            results["Exp-trig orth"] = np.dot(exp_arr, trig_arr) / (
                np.linalg.norm(exp_arr) * np.linalg.norm(trig_arr)
            )

        # Correlation matrix of parameters
        results["Correlation mtrx"] = pcov / np.sqrt(
            np.expand_dims(np.diag(pcov), 0) * np.expand_dims(np.diag(pcov), 1)
        )
        ...
        if calc_ortho:
            ...
            if iterations == 50:
                raise ValueError(
                    "Maximum orthogonality could not be determined within 50 iterations. \n"
    ...
    def exp_trig_ortho(self, R, phase, freq, a):
        ...
        projection = (
            np.exp(R) * (R * np.sin(f + p) - f * np.cos(f + p))
            - np.exp(R * a) * (R * np.sin(f * a + p) - f * np.cos(f + p))
        ) / (f**2 + R**2)

        norm_exp = np.sqrt((np.exp(2 * R) - np.exp(2 * R * a)) / (2 * R))

        norm_trig = np.sqrt(
            (2 * f * (1 - a) + np.sin(2 * (f * a + p)) - np.sin(2 * (f + p))) / (4 * f)
        )
