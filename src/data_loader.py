...
class DataLoader(Base):
    ...
    def __init__(self, folder_location=None, download_folder=r"C:\Users\joche\Downloads"):
        self.folder = (
            os.path.join(self.data_location, "security")
            if folder_location is None
            else folder_location
                ...
                if len(soup.findAll("Worksheet")) == 0:
                    ...
                    for sheet in soup.findAll(sheetflag):
                        ...
                        pd.DataFrame(sheet_as_list).to_excel(
                            writer, sheet_name=sheet.attrs[nameflag], index=False, header=False
                        )
                    ...
                    for sheet in soup.findAll("Worksheet"):
                        ...
                        for row in sheet.findAll("Row"):
                            ...
                            sheet_as_list.append(
                                [
                                    cell.Data.text if cell.Data else ""
    ...
    def perform_download(self, urls, filenames, title):
        ...
        if nelem != 0:
            ...
            #     executor.map(DataLoader.download_file, urls, filenames)
            with concurrent.futures.ThreadPoolExecutor() as executor:
                result_futures = list(
                    map(lambda x, y: executor.submit(self.download_file, x, y), urls, filenames)
                )
    ...
    def driver_download(self, url, filename):
        ...
        element = WebDriverWait(driver, 30).until(
            EC.presence_of_element_located(
                (webdriver.common.by.By.XPATH, "//select[contains(@id, 'updateTerm')]")
            )
        )
        ...
        element = WebDriverWait(driver, 30).until(
            EC.presence_of_element_located(
                (webdriver.common.by.By.XPATH, "//button[text()='Update']")
            )
        )
        element.click()
        ...
        element = WebDriverWait(driver, 30).until(
            EC.presence_of_element_located(
                (webdriver.common.by.By.XPATH, "//a[contains(text(), 'Download')]")
            )
        )
        ...
        df_last_index = nan_inds[nan_inds > 20][
            0
        ]  # Skip the set of nans at the top -> first one at the bottom

        nan_inds = new_df.isnull(df).any(0).nonzero()[0]
        new_df_last_index = nan_inds[nan_inds > 20][
            0
        ]  # Skip the set of nans at the top -> first one at the bottom
